...
export function CommonIssuesPage() {
  ...
  return <div className="page"><div className="page-head"><span className="eyebrow">Common issues</span><h1>Known complaint patterns</h1></div><SelectedVehicleCard vehicle={selectedVehicle} />{!selectedVehicle && <div className="empty-card">Select a vehicle first.</div>}{loading && <div className="state loading">Checking common issues...</div>}{error && <div className="state error">{error}</div>}<CommonIssueCards issues={issues} /></div>;
}
