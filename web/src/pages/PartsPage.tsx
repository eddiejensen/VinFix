...
export function PartsPage() {
  ...
  return <div className="page"><div className="page-head"><span className="eyebrow">Parts</span><h1>Fitment scoped part categories</h1><p>Uses GET /parts/:make/:model/:year.</p></div><SelectedVehicleCard vehicle={selectedVehicle} />{!selectedVehicle ? <div className="empty-card">Select a vehicle first to load fitment scoped part categories.</div> : null}{error ? <div className="state error">{error}</div> : null}<div className="feature-grid">{parts.map((part) => <a className="feature-card" href={part.searchUrl} target="_blank" key={part.slug} rel="noreferrer"><span className="feature-icon">🔧</span><strong>{part.title}</strong><p>{part.searchHint}</p></a>)}</div></div>;
}
